 ...
 */
//WRITE YOUR CODE BELOW
let listing1 = {...listings[0], isSold: false, currentOwner: "Jane Doe"};
/**
 ...
 */
//WRITE YOUR CODE BELOW
console.log(`This house at ${listing1.address} is  by ${listing1.currentOwner}`)
/**
...
function realtorFees(listing) {
  let price = "";
  for(let i = 0; i < listing.price.length; i++) {
    listing.price[i] >= 0 && listing.price[i] <= 9 ? price += listing.price[i] : null;
  }
...
//struggled with finding a way to compare prices with the "$" and "," in the string, so I made a function to remove them
function cleanPrice(listing) {
  let price = "";
  for(let i = 0; i < listing.price.length; i++) {
    listing.price[i] >= 0 && listing.price[i] <= 9 ? price += listing.price[i] : null;
  }
...
const listingAscendingly = listings.sort((listingA, listingB) => cleanPrice(listingA) - cleanPrice(listingB));
/**
